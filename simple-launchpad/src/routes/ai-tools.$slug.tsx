...
function ToolPage() {
  ...
  const contextProperty = useProperty ? activeProperty : null;
    ...
    try {
      const res = await generateToolOutput({
        data: {
          ...
          ...(contextProperty ? { property: contextProperty } : {}),
        },
      });
      setOutput(res.text);
    } catch (error) {
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-xl border border-border bg-card p-7">
            <div className="space-y-5">
              ...
              {activeProperty && (
                <label className="flex items-start gap-2.5 rounded-lg border border-border bg-surface px-4 py-3 text-sm">
                  <input
                    type="checkbox"
                    className="mt-1 accent-[var(--color-primary)]"
                    checked={useProperty}
                    onChange={(e) => setUseProperty(e.target.checked)}
                  />
                  <span>
                    Use my active property as context
                    <span className="block text-xs text-muted-foreground">
                      {activeProperty.name} — {activeProperty.city}
                    </span>
                  </span>
                </label>
              )}
