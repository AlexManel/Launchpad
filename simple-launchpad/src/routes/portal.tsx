...
import { PropertiesPanel } from "@/components/property/PropertiesPanel";
...
function Overview({ name, onNavigate }: { name: string; onNavigate: (s: SectionId) => void }) {
  const { properties, activeProperty } = useProperties();
  const stats = [
    ...
    { label: "Digital Products", value: "3", note: "owned", to: "products" as SectionId },
    {
      ...
      note: activeProperty ? `active: ${activeProperty.name}` : "saved",
