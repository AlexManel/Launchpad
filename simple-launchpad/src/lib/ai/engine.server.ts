...
import { getPropertyContext, renderPropertyContext } from "@/lib/property/context";
...
export async function generateAI(request: AiGenerationRequest): Promise<AiGenerationResult> {
  try {
    ...
    const propertyContext = request.property
      ? renderPropertyContext(getPropertyContext(request.property, request.tool))
      : undefined;

    const result = streamText({
      model,
      system: getSystemPrompt(request.tool),
      prompt: buildUserPrompt(request.tool, request.input, request.extra, propertyContext),
