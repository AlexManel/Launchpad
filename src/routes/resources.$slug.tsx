...
const extraArticles = {
  "review-facts-the-guest-left-out": omittedFactsEn,
  "apantisi-kakis-kritikis": omittedFactsEl,
};
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:title", content: `${resource.title} — Webrya` },
        { property: "og:description", content: resource.excerpt },
        { property: "og:type", content: "article" },
        { name: "twitter:card", content: "summary_large_image" },
        { name: "author", content: "Alex Manelidis" },
      ],
      links: [{ rel: "canonical", href: `https://webrya.com/resources/${resource.slug}` }],
    };
  },
    ...
    <>
      <div className="border-b border-border bg-surface">
        <div className="mx-auto max-w-4xl px-5 py-10 lg:px-8 lg:py-14">
          <Link to="/resources" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground">
          ...
          <div className="mt-8">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-relaxed text-muted-foreground">{excerpt}</p>
            <div className="mt-6 flex items-center gap-4 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                <Clock className="size-4" />
                {resource.readTime} {tr(t, "r.read", "read")}
              </span>
              <span>Alex Manelidis · {tr(t, "r.editorial", "Webrya Editorial")}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-3xl">
                ...
                {section.bullets && (
                  <ul className="mt-5 space-y-3">
                    {section.bullets.map((bullet) => (
                      <li key={bullet} className="pl-1 text-base leading-7 text-muted-foreground">{bullet}</li>
                    ))}
                  </ul>
                )}
                {section.steps && (
              ...
              <div className="not-prose mt-12 rounded-2xl border border-border bg-surface p-7">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{article.tool.description}</p>
                    <Button asChild className="mt-5">
                      <Link to="/ai-tools/$slug" params={{ slug: article.tool.slug }}>
                        {article.tool.cta}
          ...
          {relatedResources.length > 0 && (
            <div className="mt-16 border-t border-border pt-10">
              ...
              <h2 className="mt-3 text-2xl">{tr(t, "r.moreHosts", "More resources for hosts")}</h2>
              <div className="mt-6 grid gap-4">
                {relatedResources.map((related) => (
                  <Link key={related!.slug} to="/resources/$slug" params={{ slug: related!.slug }} className="group rounded-xl border border-border bg-card p-5 transition-shadow hover:shadow-[var(--shadow-card)]">
                    <p className="text-xs text-muted-foreground">
                      {tr(t, `cat.${related!.category}`, related!.category)} · {related!.readTime} {tr(t, "r.read", "read")}
                    </p>
                    <div className="mt-2 flex items-center justify-between gap-4">
                      <h3 className="font-medium group-hover:underline">{tr(t, `r.${related!.slug}.title`, related!.title)}</h3>
          ...
          <div className="mt-12">
            <Link to="/resources" className="inline-flex items-center gap-2 text-sm font-medium text-accent">
              <ArrowLeft className="size-4" />
